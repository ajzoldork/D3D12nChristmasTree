//*********************************************************
// This application, which is modeled off of Microsofts
// D3D12nBodyGravity sample, creates a Christmas Tree of
// rotating rings made of point sprites. Camera will auto 
// move after 30 seconds.
// 
// Keys: A, D -> x directions
//       W, S -> z directions
// 
//       Use: up & down arrow keys to control pitch,
//            left & right arrow keys to control yaw
//  
//*********************************************************
...
const float D3D12nChristmasTree::ParticleSpread = 1600.0f;

D3D12nChristmasTree::D3D12nChristmasTree(UINT width, UINT height, std::wstring name) :
...
void D3D12nChristmasTree::OnInit()
{
    m_camera.Init({ 0.0f, 50.0f, 2000.0f }); //0 0 1500
    m_camera.SetMoveSpeed(750.0f);
...
void D3D12nChristmasTree::LoadPipeline()
{
...
void D3D12nChristmasTree::LoadAssets()
{
    // Create the root signatures.
    {
        ...
        // Compute root signature.
        {
            ...
            ranges[2].Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 1, 0, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_VOLATILE);

            // added ComputeRootUAVTable2
            CD3DX12_ROOT_PARAMETER1 rootParameters[ComputeRootParametersCount];
            rootParameters[ComputeRootConstant].InitAsConstants(1, 1); // , 0, D3D12_SHADER_VISIBILITY_ALL);
            ...
            rootParameters[ComputeRootUAVTable2].InitAsDescriptorTable(1, &ranges[2], D3D12_SHADER_VISIBILITY_ALL);

            CD3DX12_VERSIONED_ROOT_SIGNATURE_DESC computeRootSignatureDesc;
            computeRootSignatureDesc.Init_1_1(_countof(rootParameters), rootParameters, 0, nullptr, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
    ...
    // Create the pipeline states, which includes compiling and loading shaders.
    {
        ...
        ThrowIfFailed(D3DCompileFromFile(GetAssetFullPath(L"nCircleCS.hlsl").c_str(), nullptr, nullptr, "CSMain", "cs_5_0", compileFlags, 0, &computeShader, nullptr));
    ...
    // Create the compute shader's constant buffer.
    {
        ...
        constantBufferCS.param[1] = int(ceil(ParticleCount / GPUxThreads));
...
void D3D12nChristmasTree::CreateVertexBuffer()
{
    ...
    {
        //* the final color is set in the pixel shader
...
float D3D12nChristmasTree::RandomPercent()
{
...
void D3D12nChristmasTree::LoadParticles2(_Out_writes_(numParticles) Particle2* pParticles, const XMFLOAT4& velocity, const bool onTree, UINT numParticles)
{
    ...
    for (UINT i = 0; i < r; i++)
    {
        for (UINT j = 0; j < GPUxThreads; j++)
        {
            float radius = 200 - (i * 5.5); 

            pParticles[k].position.x = radius * (float)sin((((360.0f / GPUxThreads) * k) * XM_PI / 180));
            float y = (float(i) * 20.0f);
            pParticles[k].position.y = y;
            pParticles[k].position.z = radius * (float)cos((((360.0f / GPUxThreads) * k) * XM_PI / 180));
            pParticles[k].position.w = 1.0f;

            pParticles[k].velocity = velocity;

            pParticles[k].onTree = onTree;
            k++;
        }
    }
    ...
    for (UINT i = 0; i < 12; ++i)
    {
        pParticles[k].position.x = pos[i].x;
        pParticles[k].position.y = pos[i].y;
        pParticles[k].position.z = pos[i].z;
        pParticles[k].position.w = 1.0f;

        pParticles[k].velocity = XMFLOAT4(0, 0, 0, 1);

        XMVECTOR n = XMVector4Normalize(XMLoadFloat4(&pParticles[k].position));
        XMVECTOR p = s * n;
        XMStoreFloat4(&pParticles[k].position, p);
        pParticles[k].position.y += 33.0f * 20.0f;

        pParticles[k].onTree = false;
...
//* 
//* Calculate the velocity/direction vector of each particle
//*
void D3D12nChristmasTree::CalcVel(_Out_writes_(numParticles) Particle* pParticles, Particle2* pParticles2, UINT numParticles)
{
    
    int k = 0;
    for (UINT i = 0; i < numParticles; i++)
    {
        XMVECTOR s = XMLoadFloat4(&pParticles[i].position);
        XMVECTOR t = XMLoadFloat4(&pParticles2[i].position);
        XMVECTOR v = t - s;
        XMVECTOR n;
        // use case: control the velocity different from the tree and its top
        // To do: velocity of each particle could be differnt
        if (i < 1024)
        {
            n = XMVector4Normalize(v) * .5;
        }
        else
        {
            n = XMVector4Normalize(v) * .5;
        ...
        XMStoreFloat4(&pParticles[i].velocity, n);
        k++;
    }
}
...
void D3D12nChristmasTree::CreateParticleBuffers()
{
    ...
    // added
    std::vector<Particle2> data2;
    data2.resize(ParticleCount);
    const UINT data2Size = ParticleCount * sizeof(Particle2);
    ...
    // added
    D3D12_RESOURCE_DESC bufferDesc2 = CD3DX12_RESOURCE_DESC::Buffer(data2Size, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    D3D12_RESOURCE_DESC uploadBufferDesc2 = CD3DX12_RESOURCE_DESC::Buffer(data2Size);
    ...
    for (UINT index = 0; index < ThreadCount; index++)
    {
        ...
        ThrowIfFailed(m_device->CreateCommittedResource(
            ...
            D3D12_RESOURCE_STATE_COPY_DEST,
            nullptr,
            IID_PPV_ARGS(&m_particleBuffer2[index])));
        ...
        ThrowIfFailed(m_device->CreateCommittedResource(
            ...
            D3D12_RESOURCE_STATE_GENERIC_READ,
            nullptr,
            IID_PPV_ARGS(&m_particleBuffer2Upload[index])));

        ThrowIfFailed(m_device->CreateCommittedResource(
            &CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_READBACK),
            D3D12_HEAP_FLAG_NONE,
            &CD3DX12_RESOURCE_DESC::Buffer(data2Size),
            D3D12_RESOURCE_STATE_COPY_DEST,
            nullptr,
            IID_PPV_ARGS(&m_ReadBackBuffer[index])));

        NAME_D3D12_OBJECT_INDEXED(m_particleBuffer0, index);
        NAME_D3D12_OBJECT_INDEXED(m_particleBuffer1, index);
        // added m_particleBuffer2
        NAME_D3D12_OBJECT_INDEXED(m_particleBuffer2, index);
        // added 
        ...
        D3D12_SUBRESOURCE_DATA particleData2 = {};
        particleData2.pData = reinterpret_cast<UINT8*>(&data2[0]);
        particleData2.RowPitch = dataSize;
        particleData2.SlicePitch = particleData.RowPitch;
        ...
        // added m_particleBuffer2
        UpdateSubresources<1>(m_commandList.Get(), m_particleBuffer2[index].Get(), m_particleBuffer2Upload[index].Get(), 0, 0, 1, &particleData2); 
        ...
        m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_particleBuffer2[index].Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
        ...
        uavDesc2.Buffer.NumElements = ParticleCount;
        uavDesc2.Buffer.StructureByteStride = sizeof(Particle2);
        uavDesc2.Buffer.CounterOffsetInBytes = 0;
        ...
        // added
        CD3DX12_CPU_DESCRIPTOR_HANDLE uavHandle2(m_srvUavHeap->GetCPUDescriptorHandleForHeapStart(), UavParticlePosVelo2 + index, m_srvUavDescriptorSize);
        ...
        // added
        m_device->CreateUnorderedAccessView(m_particleBuffer2[index].Get(), nullptr, &uavDesc2, uavHandle2);
    }
}

void D3D12nChristmasTree::CreateAsyncContexts()
{
...
void D3D12nChristmasTree::OnUpdate()
{
    // Wait for the previous Present to complete.
    WaitForSingleObjectEx(m_swapChainEvent, 100, FALSE);
    
    if (TreeIsUp && cs == CameraInit)
    {
        m_movetimer.Tick(NULL);
        if (m_movetimer.GetTotalSeconds() > 30)
        {
            cs = CameraMoving;
        }
    }
    ...
    if (cs == CameraMoving)
    {
        ...
        {
            pos.z -= 3.9f;
            m_camera.SetPosition({ 0.0f, 50.0f, pos.z });
        }
    }
...
// Render the scene.
void D3D12nChristmasTree::OnRender()
{
    try
    {
...
void D3D12nChristmasTree::ReleaseD3DResources()
{
...
void D3D12nChristmasTree::RestoreD3DResources()
{
...
void D3D12nChristmasTree::WaitForGpu()
{
...
void D3D12nChristmasTree::PopulateCommandList()
{
...
DWORD D3D12nChristmasTree::AsyncComputeThreadProc(int threadIndex)
{
    ...
    while (0 == InterlockedGetValue(&m_terminating))
    {
        ...
        // Read the buffer updated by the GPU in order to get the flags to see if the tree is completed forming
        //
        pCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_particleBuffer2[threadIndex].Get(),
            D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_SOURCE));

        pCommandList->CopyResource(m_ReadBackBuffer[threadIndex].Get(), m_particleBuffer2[threadIndex].Get());
        ...
        Particle2* mappedData = nullptr;
        ThrowIfFailed(m_ReadBackBuffer[threadIndex]->Map(0, nullptr, reinterpret_cast<void**>(&mappedData)));

        TreeIsUp = true;

        for (int i = 0; i < ParticleCount; ++i)
        {
            if (mappedData[i].onTree)
                continue;
            else
            {
                TreeIsUp = false;
        ...
        m_ReadBackBuffer[threadIndex]->Unmap(0, nullptr);
    }
...
void D3D12nChristmasTree::Simulate(UINT threadIndex)
{
    ID3D12GraphicsCommandList* pCommandList = m_computeCommandList[threadIndex].Get();
    ...
    // added uavHandle2
    CD3DX12_GPU_DESCRIPTOR_HANDLE uavHandle2(m_srvUavHeap->GetGPUDescriptorHandleForHeapStart(), UavParticlePosVelo2 + threadIndex, m_srvUavDescriptorSize);

    float t = (float)m_timer.GetTotalSeconds();
    pCommandList->SetComputeRoot32BitConstants(0, 1, &t, 0);
    ...
    // added ComputeRootUAVTable2, uavHandle2
    pCommandList->SetComputeRootDescriptorTable(ComputeRootUAVTable2, uavHandle2);

    int d = static_cast<int>(ceil(ParticleCount / GPUxThreads));
    pCommandList->Dispatch(d, 1, 1);
...
void D3D12nChristmasTree::OnDestroy()
{
...
void D3D12nChristmasTree::OnKeyDown(UINT8 key)
{
    m_camera.OnKeyDown(key);
}

void D3D12nChristmasTree::OnKeyUp(UINT8 key)
{
    m_camera.OnKeyUp(key);
}

void D3D12nChristmasTree::WaitForRenderContext()
{
...
void D3D12nChristmasTree::MoveToNextFrame()
{
